...
def insert_toCol(client, filename, docName: str, exists: bool = False, TS_colPtr = None, Scores_colPtr = None):
	...
	if(not exists):
		...
		if(len(df.columns) < 2):
			name_dict = {'meta_data': {'name': str(docName)}}
			Scores_colPtr.insert_one(name_dict)
			df = set_DatetoIndex_noTS_CSV(filename)
			data_dict = df.to_dict('index')
			name_dict.update(data_dict)
			TS_colPtr.insert_one(name_dict)
		else:
			name_dict = {'meta_data': {'name': str(docName)}}
			Scores_colPtr.insert_one(name_dict)
			df = set_timeStamptoIndex(filename)		
...
def get_dataSetNames(db = mongo.db) -> list:
	# returns list of dataset names in result
	empty = True
	dict_results = dict()
	...
	for i in range(0, len(collections), 1):
		...
		cur = col.find(filter, {'_id': 0, 'meta_data.name': 1})
		for doc in cur:
				empty = False
